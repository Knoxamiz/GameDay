...
import {
  ...
  updateParentTransportationStatus,
...
export async function PATCH(
  ...
  try {
    const entry = await updateParentTransportationStatus(
      eventId,
    ...
    console.warn("Parent transportation update failed.", {
    ...
    return NextResponse.json(
      {
        error:
          status >= 500
            ? "Could not update transportation. Please try again."
